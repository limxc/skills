# spec2md-pipeline Specification

## Purpose
TBD - created by archiving change refactor-spec2md. Update Purpose after archive.
## Requirements
...
### Requirement: Final draft output to spec2md/
The system SHALL write the confirmed final draft to `spec2md/{change-name}-{date}/article.md`, along with diagram images and drawio source files. After output, the system SHALL append a link entry to README.md under `## 需求时间线`.
...
- **THEN** system appends `- [文章标题 - YYYY-MM-DD](spec2md/{article-name}/article.md)` to README.md under `## 需求时间线`
