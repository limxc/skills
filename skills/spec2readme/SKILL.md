...
失败 → 记录 `$isGit = $false`，日期回退到文件夹创建时间。成功 → `$isGit = $true`。
...
$changePath = if (Test-Path "openspec/changes/$changeName") { "openspec/changes/$changeName" } else { "openspec/changes/archive/$changeName" }; $date = if ($isGit) { git log --follow --diff-filter=A --format=%aI -1 -- $changePath | ForEach-Object { if ($_) { ($_ -split 'T')[0] } } } else { (Get-Item $changePath).CreationTime.ToString("yyyy-MM-dd") }
...
| 1.4 | git 不可用 | $isGit = $false，日期回退到文件夹创建时间 | 不影响后续步骤 |
