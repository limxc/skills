...
基于 Step 4.1 的 `change_material_summary`，使用以下**内容方向检查表**逐项扫描，找出所有配图方向。不得凭"感觉这个不重要"跳过任何方向。

| 内容方向 | 触发关键词 | 命中条件 |
|---|---|---|
| 架构/组件关系 | 端、Web端、客户端、服务端、网关、服务器、服务、组件、模块、系统、集群、节点、Pod、容器、数据库、缓存、Redis、MySQL、消息队列、MQ | 出现 ≥2 个不同的触发词 |
| 消息传递/交互 | 调用、请求、响应、推送、发送、接收、回调、通知、广播、同步、异步、API、接口、事件、消息、网关转发、同时推送给 | 出现任意触发词，且涉及 ≥2 个实体/组件 |
| 数据模型/实体 | 表、实体、字段、属性、列、schema、模型、Model、外键、关联、关系型、ER | 出现任意触发词 |
| 工作流/流程 | 步骤、流程、流水线、pipeline、审批、审核、CI/CD、部署、发布流程、业务步骤、操作步骤 | 出现触发词，描述人/系统按顺序执行操作 |
| 状态/生命周期 | 状态、生命周期、状态机、转换、变更、待处理、进行中、已完成、已取消、待审核 | 出现 ≥2 个不同状态词 |
| 里程碑/时间线 | 里程碑、阶段、版本、发布、上线、迭代、冲刺、sprint、甘特、时间表、截止日期 | 出现任意触发词 + 时间/日期描述 |

**关键区分规则**：
- "用户填写表单→提交"是业务流 → 归"工作流/流程"
- "A 把 X 推送给 B"是系统通信 → 归"消息传递/交互"
- 出现"推送/调用/发送给"等词时，优先检查"消息传递/交互"

**输出格式**：命中方向清单（方向名 + 触发词证据）

```
命中方向：
1. 架构/组件关系 —— 证据：change 中提到"护士 Web 端、服务器、音频网关、电视屏"（4个组件）
2. 消息传递/交互 —— 证据：change 中提到"服务器将叫号事件同时推送给音频网关进行语音播报，并更新电视机屏幕"（推送、同时推送给）
3. 工作流/流程 —— 证据：change 中提到"患者到达→护士录入→生成排队号码→进入队列→医生呼叫"（多步骤顺序执行）
```

**规则**：
- 命中方向全部进入 4.2.2 展示，不得主观丢弃
- 命中方向 > 5 时，先用 question 问用户是否批量处理（全部保留 / 逐项确认 / 只保留核心方向）
- 未命中任何方向 → 直接进入 Step 5（纯文字）

**4.2.2** 为每个方向推荐配图类型并确认

根据 4.2.1 的命中方向清单，结合以下**配图类型映射表**为每个方向推荐图表类型。用户在此步骤**同时确认方向和配图类型**。

| 内容方向 | 推荐配图类型 | 备选配图类型 | 映射理由 |
|---|---|---|---|
| 架构/组件关系 | C4Context（宏观架构，≥6 个组件时优先） | flowchart（<6 个组件时更清晰） | 架构图回答"有哪些组件、怎么连" |
| 消息传递/交互 | sequenceDiagram | 无 | 时序图回答"消息怎么传、谁先调用谁" |
| 数据模型/实体 | erDiagram | classDiagram（面向对象场景） | ER 图回答"有哪些实体、什么关系" |
| 工作流/流程 | flowchart | 无 | 流程图回答"步骤怎么执行、决策分支在哪" |
| 状态/生命周期 | stateDiagram-v2 | 无 | 状态机回答"有哪些状态、怎么转换" |
| 里程碑/时间线 | gantt | 无 | 甘特图回答"什么时候做什么事" |

对每个命中方向生成选项：

```
方向：系统架构（涉及 护士 Web 端、医生 Web 端、服务器、音频网关、电视屏、科室队列）
A) 推荐：C4Context —— 内容：展示6个组件的系统级架构全景 —— 理由：组件数≥6，C4Context 适合宏观视角
B) 备选：flowchart —— 内容：展示6个组件的调用流向和依赖关系 —— 理由：组件数偏多但关系简单，flowchart 也能表达
C) 跳过这个方向

方向：消息交互（涉及 医生端、服务器、音频网关、电视屏）
A) 推荐：sequenceDiagram —— 内容：展示"呼叫下一位"事件中医生端→服务器→同时推送给音频网关和电视屏的完整时序 —— 理由：跨组件消息传递用时序图最准确
B) 跳过这个方向

方向：业务流程（患者到达→护士录入→生成号码→进入队列→医生呼叫）
A) 推荐：flowchart —— 内容：展示患者就诊叫号的完整业务步骤和决策分支 —— 理由：多步骤顺序流程用流程图表达
B) 跳过这个方向
```

对每个方向用 question 确认用户选择。
- 有备选图表的方向：展示 A)推荐 B)备选 C)跳过
- 无备选图表的方向：展示 A)保留 B)跳过

用户在不同方向间自由组合。例如：选"系统架构用 flowchart + 消息交互用 sequenceDiagram + 业务流程用 flowchart + 实体关系跳过"。
...
用户在 4.2.2 中确认的每个配图项都包含 `content` 字段（如 "微服务电商架构：Mobile/Web → API Gateway → User/Order/Product/Payment 服务 → DB + Redis"），这是**内容描述**，不是完整 prompt。4.3 需要将其**嵌入对应类型的 prompt 模板**（如下表），形成完整 prompt 后再发给 `creating-mermaid-diagrams`。关键是**具体描述组件/实体名称、流向/关系、边界/分支**。
