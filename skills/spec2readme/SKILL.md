...
基于 Step 4.1 的 `change_material_summary`，从以下维度做语义分析，找出所有可能需要配图的方向/角度。不得凭"感觉这个不重要"跳过任何方向。

分析维度：
1. **系统架构** —— change 是否涉及多个独立组件/服务/端点/系统？它们之间是否存在调用、依赖或数据流关系？
2. **消息交互** —— change 是否涉及跨组件的请求/响应/推送/调用/通知？是否有"谁发给谁"的通信链路？
3. **数据模型** —— change 是否涉及实体、表、字段、外键或对象间的关系定义？
4. **业务流程** —— change 是否涉及多步骤的顺序执行、条件判断、审批或操作分支？
5. **状态转换** —— change 是否涉及对象的生命周期、状态变更或状态机设计？
6. **时间规划** —— change 是否涉及阶段划分、里程碑、版本发布计划或工期安排？

**区分原则**：
- 业务流 vs 消息交互：描述"人/系统按步骤做事"是业务流程；描述"A 把消息推送给 B"是消息交互。两者可能同时存在，应分别列为不同方向。
- 架构 vs 消息交互：架构关注"有哪些组件、怎么连"；消息交互关注"消息怎么传、顺序是什么"。两者互补，不应合并。

**输出格式**：方向清单（方向名 + 分析理由 + 涉及的具体内容）

```
分析出的配图方向：
1. 系统架构 —— 理由：change 描述了护士Web端、服务器、音频网关、电视屏等多个独立组件及其关系
   涉及内容：护士Web端、医生Web端、服务器、音频网关、电视屏、科室队列

2. 消息交互 —— 理由：change 描述了服务器同时向音频网关和电视屏推送叫号事件的跨组件通信
   涉及内容：医生端、服务器、音频网关、电视屏

3. 业务流程 —— 理由：change 描述了患者就诊叫号的多步骤顺序流程
   涉及内容：患者到达、护士录入、生成排队号码、进入队列、医生呼叫
```

**规则**：
- 分析出的方向全部进入 4.2.2 展示，不得主观丢弃
- 方向 > 5 时，先用 question 问用户是否批量处理（全部保留 / 逐项确认 / 只保留核心方向）
- 未分析出任何方向 → 直接进入 Step 5（纯文字）

**4.2.2** 配图类型推荐与确认

根据 4.2.1 分析出的方向清单，结合 Step 4.3 的「配图类型表」为每个方向推荐图表类型。用户在此步骤**同时确认方向和配图类型**。

**映射参考**（4.2.1方向 → 4.3配图类型）：

| 4.2.1 方向 | 对应 4.3 配图类型 | 推荐图表 | 备选图表 |
|---|---|---|---|
| 系统架构 | 架构/系统 | C4Context（≥6组件） / flowchart（<6组件） | flowchart / C4Context |
| 消息交互 | 时序/协议 | sequenceDiagram | 无 |
| 数据模型 | 数据模型 | erDiagram | classDiagram |
| 业务流程 | 工作流/流程 | flowchart | 无 |
| 状态转换 | 状态机 | stateDiagram-v2 | 无 |
| 时间规划 | 甘特图/时间线 | gantt | 无 |

对每个方向生成选项：

```
方向：系统架构（涉及 护士Web端、医生Web端、服务器、音频网关、电视屏、科室队列）
A) 推荐：C4Context —— 内容：展示6个组件的系统级架构全景
   理由：C4Context 适合展示系统级组件关系（见4.3配图类型表：C4高层架构）
B) 备选：flowchart —— 内容：展示6个组件的调用流向
   理由：组件数偏多但关系简单，flowchart 也能表达（见4.3配图类型表：架构/系统）
C) 跳过这个方向

方向：消息交互（涉及 医生端、服务器、音频网关、电视屏）
A) 推荐：sequenceDiagram —— 内容：展示"呼叫下一位"的完整时序
   理由：跨组件消息传递用时序图最准确（见4.3配图类型表：时序/协议）
B) 跳过这个方向
```
