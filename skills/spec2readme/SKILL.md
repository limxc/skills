...
$changeDir = if (Test-Path "openspec/changes/$changeName") { "openspec/changes/$changeName" } else { "openspec/changes/archive/$changeName" }; $date = (Select-String -Path "$changeDir/.openspec.yaml" -Pattern "^created: " | ForEach-Object { $_.Line -replace "^created: ", "" }).Trim()
