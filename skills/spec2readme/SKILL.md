...
$changeDir = ("openspec/changes/$changeName", "openspec/changes/archive/$changeName") | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $changeDir) { Write-Error "Change directory not found: $changeName"; exit 1 }
$projectRoot = (python <skill-dir>/scripts/position.py --print-root 2>$null)
if (-not $projectRoot) { $projectRoot = (Get-Item ".").FullName }
$date = (Select-String "$changeDir/.openspec.yaml" -Pattern "^created: ").Line.Split(": ", 2)[1]
