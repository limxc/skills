...
$date = if ($isGit) { git log --follow --diff-filter=A --format=%aI -1 -- "openspec/changes/$changeName" | ForEach-Object { if ($_) { ($_ -split 'T')[0] } } } else { (Get-Item "openspec/changes/$changeName").CreationTime.ToString("yyyy-MM-dd") }
