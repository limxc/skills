...
根据选中的 change 内容和 Step 2.3 骨架类型，推荐最匹配的 3 个框架供用户选择。每套框架是一个完整的文章骨架——不是写文章本身，而是告诉写作步骤"每一段写什么、怎么写"。
...
根据 Step 2.3 骨架类型和 change 内容特征推荐 top 3 框架：

| 骨架类型 | 最佳匹配 | 次佳 | 第三 |
|---------|---------|------|------|
| SCQA（单 change 技术叙事） | 对比型 | 复盘型 | 痛点型 |
| 时间线复盘（多 change 时间序列） | 复盘型 | 故事型 | 热点解读型 |
| 对比（改造类变更） | 对比型 | 痛点型 | 清单型 |
| 清单（多不相关 feature） | 清单型 | 复盘型 | 对比型 |

## 框架嵌入方式

匹配到框架后，将其结构作为写作的段落组织参考。**不得将框架名称直接用作章节标题**。写入 article.md 时所有标题须重新拟定为自然标题。

框架的每个 H2 段落对应一个自然标题，框架的"金句落点"作为该段落的情绪密度目标，框架的"结尾策略"作为结束段的参考，但最终收尾方式须根据文章实际内容判断。
